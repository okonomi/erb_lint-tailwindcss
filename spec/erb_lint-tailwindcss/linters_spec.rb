...
RSpec.describe "ERBLint::Linters::Tailwind" do
  describe "linter classes" do
    it "ClassOrder can be instantiated" do
      linter = ERBLint::Linters::Tailwind::ClassOrder.new(nil, {})
      expect(linter).to be_a(ERBLint::Linters::Tailwind::ClassOrder)
    end

    it "Duplicate can be instantiated" do
      linter = ERBLint::Linters::Tailwind::Duplicate.new(nil, {})
      expect(linter).to be_a(ERBLint::Linters::Tailwind::Duplicate)
    end

    it "Unknown can be instantiated" do
      linter = ERBLint::Linters::Tailwind::Unknown.new(nil, {})
      expect(linter).to be_a(ERBLint::Linters::Tailwind::Unknown)
    end
  end

  describe "linter configuration" do
    it "ClassOrder has proper config schema" do
      schema = ERBLint::Linters::Tailwind::ClassOrder::CONFIG_SCHEMA
    ...
    it "Duplicate has proper config schema" do
      schema = ERBLint::Linters::Tailwind::Duplicate::CONFIG_SCHEMA
      expect(schema).to be_a(Hash)
      expect(schema[:properties]).to have_key(:dedupe_across_variants)
    end

    it "Unknown has proper config schema" do
      schema = ERBLint::Linters::Tailwind::Unknown::CONFIG_SCHEMA
      expect(schema).to be_a(Hash)
  ...
  describe "linter methods" do
    let(:class_order) { ERBLint::Linters::Tailwind::ClassOrder.new(nil, {}) }
    let(:duplicate) { ERBLint::Linters::Tailwind::Duplicate.new(nil, {}) }
    let(:unknown) { ERBLint::Linters::Tailwind::Unknown.new(nil, {}) }
